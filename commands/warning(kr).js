...
module.exports = {
    ...
    execute(message,args){
        ...
        if(args[0] == null) return message.reply(embed.setColor("#F8C300").setTitle("경고 명령어 사용법").addFields({name:"사용법",value:"^warn <TYPE> <Target>"}
        ...
        switch(args[0]){
            case "부여":
                ...
                var reason = "없음";
                return message.reply(embed.setColor("#fd0061").setTitle("완료").setDescription(`구구!:dove: 성공적으로 ${target.user.username}님에게 경고 1회를 부여하였습니다`).addFields({name:"경고 사유",value:reason},
            ...
            case "회수":
                ...
                warnings[target.id]--
                return message.reply(embed.setTitle("구구!:dove: 완료").setColor("#00D166").addField(`${target.user.username}에게서 경고를 회수했습니다`,`${target.user.username}의 경고 횟수: ${warnings[target.id]}회`))
                }) 
            case "확인":
                if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply("문제 발생!\n:x:당신은 관리자 권한이 없습니다")
                if(!message.mentions.members.first()) return message.reply(embed.setTitle("에러!").setDescription("멤버를 지정해주세요!").setColor("#fc4732"))
                ...
                return message.reply(embed.setTitle(`구구!:dove: ${target.user.username}의 경고 횟수`).setDescription(`${warnings[target.id]}회`).setColor("#0099e1"))
        }
    }
}