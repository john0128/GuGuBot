...
module.exports = {
    ...
    execute(message,args){
        ...
        if(args[0] == null) return message.reply(embed.setColor("#F8C300").setTitle("Usage of warn command").addFields({name:"Usage",value:"^warn <TYPE> <Target>"}
        ...
        switch(args[0]){
            case "give":
                ...
                var reason = "None";
                return message.reply(embed.setColor("#fd0061").setTitle("GuGu!:dove: Complete!").setDescription(`I gave ${target.user.username} Warning Successfully!`).addFields({name:"warning reason",value:reason},
                {name:`${target.user.username}'s warning count`,value:`Warning Count : ${warnings[target.id]}`}))
            case "remove":
                if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply("GuGu!:dove: ERROR!\n:x:You don't have Permission of ADMIN")
                ...
                warnings[target.id]--
                return message.reply(embed.setTitle("GuGu!:dove: Complete").setColor("#00D166").addField(`Deleted ${target.user.username}'s Warning Successfully`,`${target.user.username}\'s Warning Count: ${warnings[target.id]}`))
                case "check":
                    if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply("Error Occured!\n:x:You don't have Administrator")
                    if(!message.mentions.members.first()) return message.reply(embed.setTitle("Error!").setDescription("Please Designate Member!").setColor("#fc4732"))
                    ...
                    return message.reply(embed.setTitle(`GuGu!:dove: ${target.user.username}'s Warning Count`).setDescription(`Count : ${warnings[target.id]}`).setColor("#0099e1"))       
        }
    }
}